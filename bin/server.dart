...
class Room{
  ...
  void checkWinner(List<String> displayXO, var player1, var player2){
    ...
    if (displayXO[0] == displayXO[1] &&
      ...
      winnerFound = true;
      if(displayXO[0] == 'X'){
        player1.player!.add(jsonEncode({'type': 'game_over', 'winner': 'Player X', 'board': displayXO}));
        player2.player!.add(jsonEncode({'type': 'game_over', 'winner': 'Player X', 'board': displayXO}));
        print('X wins!');
      }else{
        player1.player!.add(jsonEncode({'type': 'game_over', 'winner': 'Player O', 'board': displayXO}));
        player2.player!.add(jsonEncode({'type': 'game_over', 'winner': 'Player O', 'board': displayXO}));
        print('O wins!');
      }
    }
    ...
    if(winnerFound == false && filledBox == 8){
      player1.player!.add(jsonEncode({'type': 'game_over', 'winner': 'Ничья', 'board': displayXO}));
      player2.player!.add(jsonEncode({'type': 'game_over', 'winner': 'Ничья', 'board': displayXO}));
      print('Nobody wins!');
    }
    filledBox++;
  }
...
void main() async {
  ...
  print('Server started on port ${server.address}:${server.port}');
